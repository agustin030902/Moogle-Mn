...
{	private string strTexto; //Texto original de la consulta
 	private Dictionary<string,int> lTerminos = new Dictionary<string,int>(); //Diccionario donde se almacenan los terminos que aparecen en la consulta y la cantidad de veces que apartece cada uno
	...
	{
		this.strTexto = texto;//guardamos el texto
 	...
 	public List<string> TerminosProhibidos
	{
 	...
 	//Devuelve el diccionario donde se almacenan los terminos importantes que aparecen en la consulta 
	...
	{   List<string> palabras = new();
		...
		string ultimotermino = String.Empty;//ultimo termino procesado
	 	bool distanciaentrepalabras = false;
	 
	 	palabras = Herramientas.SeparaPalabras(strTexto, Herramientas.OperadoresBusqueda); //Se crea una lista de las palabra que contiene la consulta respetando los operadores de busqueda
	 	foreach( string palabra in palabras )
		{
			switch (palabra[0]){//se separan los casos por el primer caracter de la palabra
        		case '!'://caso de termino prohibido
					temp = palabra.Substring(1,palabra.Length-1);
            		if (!Herramientas.EsPalabraVacia(temp))
					{
						...
						ultimotermino=temp;	//se guarda el último termino procesado para usarse despues
					}
            	break;
				...
				case '*': //caso de termino importante
					...
					do {
						temp = palabra[contador..];//al finalizar el ciclo, temp contiene el termino
						contador++;
					} while (temp[0]== '*');
