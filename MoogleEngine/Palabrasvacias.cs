...
{	
	...
	private static List<string> PalabrasVacias = new();//listado de palabras vacías
	...
	{ 
		...
		PalabrasVacias.AddRange(Utiles.Articulos);
		PalabrasVacias.AddRange(Utiles.Preposiciones);
		PalabrasVacias.AddRange(Utiles.Conjunciones);
		PalabrasVacias.AddRange(Utiles.Pronombres);
		PalabrasVacias.AddRange(Utiles.Adverbios);
		...
		var ContadorPalabrasOrdenado = ContadorPalabras.OrderBy(x => x.Value).ToDictionary(x => x.Key, x => x.Value);//ordenamos las palabras de menor a mayor por la cantidad de veces q aparecen 
